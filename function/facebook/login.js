...
if ( event.logMessageType == "log:subscribe" ){
	if (event.logMessageData.addedParticipants.some(id => id.userFbId == api.getCurrentUserID())) {
		return api.sendMessage(`Touka Connected! Prefix : ${global.config.prefix}`, event.threadID, () => {
		...
		if (userID !== api.getCurrentUserID()) {
			...
			api.sendMessage(`Welcome ${userMentions} join group ${threadName}`, event.threadID);
		}
  })
}
logsub ()
...
} else if ( event.logMessageType == "log:unsubscribe" ) {
{
	...
	api.getUserInfo([userID], (err, userInfo) => {
		var userMentions = `${userInfo[userID].name}`;
		if (userID !== api.getCurrentUserID()) {
			api.sendMessage(`Bye ${userMentions} has left ${threadName}`, event.threadID);
		}
	})
}
logunsub ()
...
async function checkupdate() {
	try {
		...
		if (data.version != global.package.version) {
			console.log("There is a new update OwO");
		} else {
		console.log("You are using the latest version UwU");
		}
	} catch(e) {
...
async function load(api){
    ...
    console.log(`[DONE COMMAND] Loaded Successfully : ${files.length} lệnh!`);
    console.log("Bot starts to receive messages\n");
	console.logg("     /$$$$$  /$$$$$$  /$$$$$$$  /$$$$$$$\n    |__  $$ /$$__  $$| $$__  $$| $$__  $$\n       | $$| $$  \\ $$| $$  \\ $$| $$  \\ $$\n       | $$| $$$$$$$$| $$$$$$$ | $$  | $$\n  /$$  | $$| $$__  $$| $$__  $$| $$  | $$\n | $$  | $$| $$  | $$| $$  \\ $$| $$  | $$\n |  $$$$$$/| $$  | $$| $$$$$$$/| $$$$$$$/\n  \\______/ |__/  |__/|_______/ |_______/\n");
	console.logg("Don't ByPass!  Please Respect Our Project....\n\n")
