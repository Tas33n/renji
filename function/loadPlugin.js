...
function loadfunc(){
    ...
    for(var i=0; i<listFile.length; i++){
        try{
            ...
            console.log("Unable to load \""+listFile[i]+"\" with error: "+err)
        }
    }
}
...
function load(file, pluginInfo){
    //try{
        ...
        console.log("Commands Successfully Loaded : "+pluginInfo.name+" "+pluginInfo.version+" by "+pluginInfo.author)
    //}
    /*catch(err){
    ...
    if(typeof pluginInfo.nodeDepends == "object"){
        for (var i in pluginInfo.nodeDepends){
            if (!fs.existsSync(path.join(__dirname, "..", "node_modules", i, "package.json"))) {
                ...
                console.log("Install Modules \""+i+"\" for commands \""+pluginInfo.name+"\":\n");
