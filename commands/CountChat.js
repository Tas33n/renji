...
async function countchat (event, api){
	...
	if (IDs.length != 0){
		...
		api.sendMessage(`Total messages of ${userName} in the group is : ${!global.data.countchat[event.threadID][IDs[0]] ? "0" : global.data.countchat[event.threadID][IDs[0]]}`, event.threadID, event.messageID);
	}
	else if (args == ""){
		api.sendMessage(`The total number of your messages in the group is : ${global.data.countchat[event.threadID][event.senderID]}`, event.threadID, event.messageID);
	}
	else if (args == "all"){
	var speech = ``;
	for(i=1;i<Object.keys(global.data.countchat[event.threadID]).length+1;i++){
		...
		var userName = await user[Object.keys(global.data.countchat[event.threadID])[i-1]].name
			speech += `${i}. ${userName}: ${Object.values(global.data.countchat[event.threadID])[i-1]} message\r\n`;
	}
